...
class EchoCancellation(buffer.Buffering):
    def __init__(self):
        super().__init__()
        self.delay_estimation = 0  # Estimated delay (d)
        self.alpha_estimation = 1  # Estimated attenuation factor (a)
        self.i = 0
        self.pulse_sent = None
        self.estimate_thread = None  # Thread for delay and attenuation estimation
    ...
    def estimate_delay_and_attenuation(self, sent_signal, received_signal):
        ...
        # Concatenate two chunks for better calculation
    ...
    def _record_IO_and_play(self, ADC, DAC, frames, time, status):
        ...
        with self.lock:
            delay = self.delay_estimation
            attenuation = self.alpha_estimation
